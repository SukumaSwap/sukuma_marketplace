...
using SafeMathUpgradeable for uint256;
    ...
    function createOffer(
        ...
        // incrementing the offerCounter for each new offer
        offerCounter++;
...
//helper function to allow withdraw function to access users's trades  
function getTrades(address token, address user) internal  returns (Trade memory) {
  for (uint i = 0; i < tradeCounter; ) {

    if (trades[i].token == token && trades[i].sender == user) {
      return trades[i];
    }
    unchecked { ++i; }
  }
  }
//helper function to allow withdraw function to access users's offers 
function getOffers(address token, address user) internal  returns (Offer memory) {
  for (uint i = 0; i < offerCounter; ) {

    if (offers[i].token == token && offers[i].owner == user) {
      return offers[i];
    }
    unchecked { ++i; }
  }
...
function getAccounts(address walletAddress) internal view returns (address,uint256, uint256, uint256, uint256, mapping(address => uint256) storage) {
    for (uint i = 0; i < nextAccountId; ) {
        if (accounts[i].walletAddress == walletAddress) {
            return (accounts[i].accountId, accounts[i].likes, accounts[i].dislikes, accounts[i].blocks, accounts[i].balance);
        }
        unchecked { ++i; }
    }
}

    //withdraw function: restricts 2 kinds of users 1. Creator of SellTrade and Trade is active .. 
    //2.creator of Sell Offer and balance is equal to offers min value
   ...
   Trade memory trade = getTrades(_token, msg.sender);     
   Offer memory offer = getOffers(_token, msg.sender); 
// Use the modified getAccount function to get the account details
   (address walletAddress,uint256 accountId, uint256 likes, uint256 dislikes, uint256 blocks, mapping(address => uint256) storage balances) = getAccounts(walletAddress);(msg.sender);
 
        if (
            msg.sender == trade.sender 
            &&
        ...
        // Check if the sender is the creator of a sell offer and the account balance is equal to the minimum offer amount
            ...
            && 
            offer.offerType==OfferType.Sell  //confirms if the one whocal withdraw ,is the same one whocaled createOffer
            &&
           balances[_token] >= offer.min
            // accounts[msg.sender].balance>= offer.min
            // account.balance[_token] >= quantity
